...
SERVER_URL = "http://192.168.178.131:5000/transcribe"  # Default server URL, configurable by user
...
def main():
    ...
    # Neue Schaltflächen-Abteilung "Normalbefunde"
    normalbefunde_label = tk.Label(root, text="Normalbefunde", font=("Helvetica", 10, "bold"))
    normalbefunde_label.pack(pady=5)

    normalbefunde_frame = tk.Frame(root)
    normalbefunde_frame.pack(pady=5)

    def insert_text(text):
        additional_input_field.insert(tk.END, text)

    lunge_button = tk.Button(normalbefunde_frame, text="Lunge", 
                             command=lambda: insert_text("Lunge frei, vesikuläres Atemgeräusch, keine pathologischen Rasselgeräusche. "))
    lunge_button.grid(row=0, column=0, padx=5)

    abdomen_button = tk.Button(normalbefunde_frame, text="Abdomen", 
                               command=lambda: insert_text("Abdomen weich, Darmgeräusche regelhaft über allen vier Quadranten, keine Abwehrspannung, kein vermehrter Druckschmerz. "))
    abdomen_button.grid(row=0, column=1, padx=5)

    rachen_button = tk.Button(normalbefunde_frame, text="Rachen", 
                              command=lambda: insert_text("Rachen unauffällig, keine Tonsillenhypertrophie, keine Beläge. "))
    rachen_button.grid(row=0, column=2, padx=5)

    ohr_button = tk.Button(normalbefunde_frame, text="Ohr", 
                           command=lambda: insert_text("Trommelfell beidseits blande, keine Rötung oder Vorwölbung. "))
    ohr_button.grid(row=1, column=0, padx=5)

    herz_button = tk.Button(normalbefunde_frame, text="Herz", 
                            command=lambda: insert_text("Herz regelmäßig, normofrequent, keine vitientypischen Nebengeräusche. "))
    herz_button.grid(row=1, column=1, padx=5)

    wirbelsaeule_button = tk.Button(normalbefunde_frame, text="Wirbelsäule", 
                                    command=lambda: insert_text("Wiebelsäule ohne Klopfschmerz, Reflexe seitengleich, pDMS intakt, Lasegue negativ. "))
    wirbelsaeule_button.grid(row=1, column=2, padx=5)
